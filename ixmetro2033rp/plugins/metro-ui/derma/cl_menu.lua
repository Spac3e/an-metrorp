...
DEFINE_BASECLASS("ixMetroSubpanelParent")
local PANEL = {}
...
function PANEL:Init()
	...
	self.paddingAmount = ScreenScale(8)

	-- setup
	self:SetPadding(self.paddingAmount, true)
	self:SetSize(ScrW(), ScrH())

	local minHeight = math.max(self:GetTall() * 0.12, 120)

	self:SetPos(0, 0)
	self:SetTopOffset(minHeight)

	local material

	-- main button panel
	self.buttons = self:Add("Panel")
	self.buttons:SetSize(self:GetWide() - self:GetPadding() * 2, minHeight)
	self.buttons:Dock(TOP)
	self.buttons:SetPaintedManually(true)

	local close = self.buttons:Add("ixMetroMenuButton")
	...
	material = PLUGIN.iconWhitelist["RETURN"] or PLUGIN.iconWhitelist["DEFAULT"]

	close.icon = close:Add("DImage")
	close.icon:SetImage(material)
	close.icon:SetImageColor(color_white)
	close.icon:SetKeepAspect(true)
	close.icon:Dock(FILL)

	local characters = self.buttons:Add("ixMetroMenuButton")
	...
	material = PLUGIN.iconWhitelist["CHARACTERS"] or PLUGIN.iconWhitelist["DEFAULT"]

	characters.icon = characters:Add("DImage")
	characters.icon:SetImage(material)
	characters.icon:SetImageColor(color_white)
	characters.icon:SetKeepAspect(true)
	characters.icon:Dock(FILL)

	-- @todo make a better way to avoid clicks in the padding PLEASE
	self.guard = self:Add("Panel")
	self.guard:SetPos(0, 0)
	self.guard:SetSize(self:GetTall(), self:GetPadding())
	...
	self:CenterButtons(close)

	self:MakePopup()
	self:OnOpened()
end

function PANEL:CenterButtons(closeButton)
	local buttonwidth = 0

	local bShowingConfig = false

	for k,v in pairs(self.buttons:GetChildren()) do
		if (v:GetName() == "Panel") then continue end

		buttonwidth = (buttonwidth + v:GetWide() + 8)
	end

	for k,v in pairs(self.tabs.buttons) do
		if (v.label and v.label:GetText() == "PLUGINS") then
			continue
		end
	...
	local remainingspace = (ScrW() - buttonwidth)

	closeButton:DockMargin((remainingspace / 2) - 2, 0, 4, 0)
end
...
function PANEL:GetOverviewInfo(origin, angles, fov)
	...
	local forward = originAngles:Forward() * 65 - originAngles:Right() * 24
	forward.z = 0
	...
	if (IsValid(target)) then
		...
		newOrigin = origin - LocalPlayer():OBBCenter() * 0.5 + forward
	end
...
function PANEL:GetStandardSubpanelSize()
	return ScrW() - self:GetPadding() * 2, ScrH() - self:GetPadding() * 2 - (self:GetTall() * 0.15)
end
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	local buttonColor = Color(0, 0, 0, 255)
	...
	local width = subpanel:GetWide()
	subpanel:DockPadding(width / 12, 0, width / 12, 0)
	...
	if (sectionParent) then
		...
		button = self.tabs:Add("ixMetroMenuSelectionButton")
		...
		local _name = L(name):utf8upper()

		if (!(_name == "PLUGINS")) then
			local material = _name and PLUGIN.iconWhitelist[_name] or PLUGIN.iconWhitelist["DEFAULT"]

			button.icon = button:Add("DImage")
			button.icon:SetImage(material)
			button.icon:SetImageColor(color_white)
			button.icon:SetKeepAspect(true)
			button.icon:Dock(FILL)
		end
	end
...
function PANEL:PopulateTabs()
	...
	if (character) then
		local money = self.tabs:Add("DPanel")
		local moneyLabel  = money:Add("DLabel")

		money:DockMargin(4, 0, 4, 0)

		moneyLabel:Dock(FILL)
		moneyLabel:SetContentAlignment(5)
		moneyLabel:SetText(ix.currency.Get(character:GetMoney()))
		moneyLabel:SetFont("ixMetroMenuFont")
		moneyLabel:SetTextColor(Color(233, 121, 39))

		money:SetZPos(10001)
		money:SetWide(moneyLabel:GetWide() + 160)
		money:Dock(LEFT)
		money.Paint = function(moneyPanel, width, height)
			surface.SetDrawColor(Color(0, 0, 0, 200))
			surface.DrawRect(0, 0, width, height)
		end

		self.tabs.buttons[#self.tabs.buttons + 1] = money
	end
...
function PANEL:PerformLayout()
	self.guard:SetSize(self:GetWide(), self.tabs:GetTall() + self:GetPadding() * 2)
end
...
vgui.Register("ixMenu", PANEL, "ixMetroSubpanelParent")
