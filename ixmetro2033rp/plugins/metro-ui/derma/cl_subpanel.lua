...
vgui.Register("ixMetroSubpanel", PANEL, "EditablePanel")
...
AccessorFunc(PANEL, "topOffset", "TopOffset", FORCE_NUMBER)
...
function PANEL:AddSubpanel(name)
	...
	local panel = BaseClass.Add(self, "ixMetroSubpanel")
...
function PANEL:SetSubpanelPos(id, x)
	...
	-- @todo needs testing on different resolutions
	currentPanel:SetPos(x, self.topOffset + 40)
	...
	while (IsValid(currentPanel)) do
		...
		if (IsValid(left)) then
			left:MoveLeftOf(currentPanel, self.padding)
		end
...
function PANEL:SetActiveSubpanel(id, length)
	...
	if (length == 0 or !self.activeSubpanel) then
		self:SetSubpanelPos(id, self.padding)
	else
		local x, _ = activePanel:GetPos()
		local target = self.targetSubpanelX
		self.currentSubpanelX = x + self.padding
...
vgui.Register("ixMetroSubpanelParent", PANEL, "EditablePanel")
